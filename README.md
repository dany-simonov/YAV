...
| Видео | Проверка файла через Gemini Video Verification. |
...
- Аудио дополнительно проверяется через ffprobe. Видео не зависит от FFmpeg или ffprobe: после проверки размера, сигнатуры и типа файл сервером загружается во временный ресурс Gemini File API и передаётся Gemini Video Verification.
...
Для синхронного анализа Function также требует явно заданные server-side параметры `SYNCHRONOUS_ANALYZE_EXECUTION_TIMEOUT_SECONDS`, `SYNCHRONOUS_ANALYZE_SAFETY_MARGIN_SECONDS` и `SYNCHRONOUS_ANALYZE_RESPONSE_SAFETY_MARGIN_SECONDS`. Первый должен совпадать с фактическим timeout Function; второй резервирует время для записи результата, третий — для формирования response. Без них анализ fail-closed. Для VIDEO нужны `GEMINI_API_KEY`, HTTPS-значение `GEMINI_API_URL` и `GEMINI_MODEL`; видео загружается только сервером из уже проверенного Appwrite-файла. Gemini smoke по умолчанию выключен. Для внутренней диагностики нужны `GEMINI_SMOKE_ENABLED=true`, отдельный `GEMINI_SMOKE_DIAGNOSTIC_SECRET` и заголовок `X-YAV-Diagnostic-Authorization`; обычный пользователь не может вызвать его по имени action.
