...
const credibilityLabel = (value: unknown): string | null => {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > 1) return null;
  return `Достоверность: ${Math.round(value * 100)}%`;
};

const mediaConfidenceLabel = (kind: string, value: number): string =>
  `${kind === 'video' ? 'достоверность' : 'уверенность'} ${Math.round(value * 100)}%`;
...
export function CheckResultCard({ result }: Props) {
  ...
  const credibilityConfidence = credibility?.status === 'completed' ? credibilityLabel(credibility.confidence) : null;
  const rows = [
  ...
  const credibilityRows = credibility?.status === 'completed'
    ? [
        ...
        ...(credibilityConfidence ? [['03 · Достоверность', credibilityConfidence.replace('Достоверность: ', ''), 'text-mv-text']] : []),
  ...
  return <article className={`${isComplex ? 'complex-check-report' : ''} bg-white border border-black/[.09] rounded-[20px] shadow-[0_1px_2px_rgba(0,0,0,.04),0_24px_60px_rgba(0,0,0,.07)] overflow-hidden`}>
    ...
    {isComplex && result.source && <section className="complex-print-section px-6 sm:px-9 pb-8"><div className="border-t border-black/[.07] pt-7"><p className="eyebrow mb-3">Источник публикации</p><h3 className="text-xl font-semibold break-words">{result.source.title || 'Публичная страница'}</h3>{result.source.site_name && <p className="mt-2 text-sm text-mv-text-muted break-words">{result.source.site_name}</p>}<a className="mt-3 block text-sm text-mv-text underline underline-offset-4 break-all" href={result.source.url} target="_blank" rel="noreferrer">{result.source.url}</a>{result.source.description && <p className="mt-4 text-sm leading-6 text-mv-text-secondary break-words">{result.source.description}</p>}<div className="mt-5 grid gap-2 text-sm text-mv-text-secondary sm:grid-cols-3"><span>Текст: {result.source.text_found ? 'извлечён' : 'недостаточен'}</span><span>Изображения: {result.source.images_analyzed}</span><span>Видео: {result.source.video_analyzed ? 'проанализировано' : 'нет'}</span></div>{result.source.text_truncated && <p className="mt-3 text-xs text-mv-text-muted">Для анализа использован ограниченный фрагмент извлечённого текста.</p>}{result.source.media.length > 0 && <div className="mt-7"><p className="eyebrow mb-3">Медиаматериалы</p><div className="grid gap-3">{result.source.media.map((media) => <article className="complex-print-keep rounded-xl border border-black/[.08] p-4" key={`${media.kind}-${media.ordinal}`}><p className="font-semibold">{media.kind === 'image' ? `Изображение ${media.ordinal}` : 'Видео'}</p>{media.status === 'unavailable' ? <p className="mt-2 text-sm text-mv-text-secondary">Анализ временно недоступен</p> : <><p className={`mt-2 text-sm ${status[media.verdict || 'UNCERTAIN']}`}>{labels[media.verdict || 'UNCERTAIN']}</p><p className="mt-1 text-sm text-mv-text-secondary">Индекс подлинности: {media.authenticity_index}%{typeof media.confidence === 'number' ? ` · ${mediaConfidenceLabel(media.kind, media.confidence)}` : ''}</p>{media.model && <p className="mt-1 text-xs text-mv-text-muted">Метод: {displayModelName(media.model)}</p>}{media.explanation && <p className="mt-2 text-sm leading-6 text-mv-text-secondary break-words">{media.explanation}</p>}</>}</article>)}</div></div>}</div></section>}
    {isComplex && result.complex_media?.length ? <section className="complex-print-section px-6 sm:px-9 pb-8"><div className="border-t border-black/[.07] pt-7"><p className="eyebrow mb-3">Загруженные материалы</p><div className="grid gap-3">{result.complex_media.map((media) => <article className="complex-print-keep rounded-xl border border-black/[.08] p-4" key={`manual-${media.kind}-${media.ordinal}`}><p className="font-semibold">{media.kind === 'image' ? `Изображение ${media.ordinal}` : media.kind === 'audio' ? `Аудио ${media.ordinal}` : `Видео ${media.ordinal}`} · загружено пользователем</p>{media.status === 'unavailable' ? <p className="mt-2 text-sm text-mv-text-secondary">Анализ временно недоступен</p> : <><p className={`mt-2 text-sm ${status[media.verdict || 'UNCERTAIN']}`}>{labels[media.verdict || 'UNCERTAIN']}</p><p className="mt-1 text-sm text-mv-text-secondary">Индекс подлинности: {media.authenticity_index}%{typeof media.confidence === 'number' ? ` · ${mediaConfidenceLabel(media.kind, media.confidence)}` : ''}</p>{media.model && <p className="mt-1 text-xs text-mv-text-muted">Метод: {displayModelName(media.model)}</p>}{media.explanation && <p className="mt-2 text-sm leading-6 text-mv-text-secondary break-words">{media.explanation}</p>}</>}</article>)}</div></div></section> : null}
